...
HAL_StatusTypeDef HAL_ADCEx_Calibration_SetValue(ADC_HandleTypeDef *hadc,
	...
	if ((LL_ADC_IsEnabled(hadc->Instance) != 0UL)
		...
		LL_ADC_SetCalibrationFactor(hadc->Instance, SingleDiff,
				CalibrationFactor);
	} else {
	...
	if (LL_ADC_REG_IsConversionOngoing(hadc->Instance) != 0UL) {
		...
		if (tmp_hal_status == HAL_OK) {
			...
			tmp_hal_status = HAL_DMA_Start_IT(hadc->DMA_Handle,
					(uint32_t) &tmpADC_Common->CDR, (uint32_t) pData, Length);
...
HAL_StatusTypeDef HAL_ADCEx_InjectedConfigChannel(ADC_HandleTypeDef *hadc,
	...
	assert_param(
			IS_ADC_SINGLE_DIFFERENTIAL(pConfigInjected->InjectedSingleDiff));
	...
	assert_param(
			!((pConfigInjected->InjectedOffsetNumber != ADC_OFFSET_NONE) && (pConfigInjected->InjecOversamplingMode == ENABLE)));

	/* JDISCEN and JAUTO bits can't be set at the same time  */
	assert_param(
			!((pConfigInjected->InjectedDiscontinuousConvMode == ENABLE) && (pConfigInjected->AutoInjectedConv == ENABLE)));

	/*  DISCEN and JAUTO bits can't be set at the same time */
	assert_param(
			!((hadc->Init.DiscontinuousConvMode == ENABLE) && (pConfigInjected->AutoInjectedConv == ENABLE)));
	...
	if (pConfigInjected->InjectedSingleDiff != ADC_DIFFERENTIAL_ENDED) {
		...
		assert_param(
				IS_ADC_DIFF_CHANNEL(hadc, pConfigInjected->InjectedChannel));
	}
	...
	if ((hadc->Init.ScanConvMode == ADC_SCAN_DISABLE)
			...
			if (pConfigInjected->ExternalTrigInjecConv
				...
				tmp_jsqr_context_queue_being_built =
						(ADC_JSQR_RK(pConfigInjected->InjectedChannel,
								ADC_INJECTED_RANK_1)
								| (pConfigInjected->ExternalTrigInjecConv
										& ADC_JSQR_JEXTSEL)
		...
		tmp_jsqr_context_queue_being_built &= ~ADC_JSQR_RK(ADC_SQR3_SQ10,
				pConfigInjected->InjectedRank);
	...
	if (LL_ADC_INJ_IsConversionOngoing(hadc->Instance) == 0UL) {
		...
		if (pConfigInjected->AutoInjectedConv == DISABLE) {
			MODIFY_REG(hadc->Instance->CFGR, ADC_CFGR_JQM | ADC_CFGR_JDISCEN,
					ADC_CFGR_INJECT_CONTEXT_QUEUE((uint32_t)pConfigInjected->QueueInjectedContext) | ADC_CFGR_INJECT_DISCCONTINUOUS((uint32_t)pConfigInjected->InjectedDiscontinuousConvMode));
		}
	...
	if ((tmp_adc_is_conversion_on_going_regular == 0UL)
		...
		if (pConfigInjected->InjecOversamplingMode == ENABLE) {
			...
			assert_param(
					IS_ADC_RIGHT_BIT_SHIFT(pConfigInjected->InjecOversampling.RightBitShift));
    ...
    else
    {
      ...
      LL_ADC_SetSamplingTimeCommonConfig(hadc->Instance, LL_ADC_SAMPLINGTIME_COMMON_DEFAULT);
		...
		tmp_offset_shifted = ADC_OFFSET_SHIFT_RESOLUTION(hadc,
				pConfigInjected->InjectedOffset);
		...
		if (pConfigInjected->InjectedOffsetNumber != ADC_OFFSET_NONE) {
			...
			if (__LL_ADC_CHANNEL_TO_DECIMAL_NB(
					LL_ADC_GetOffsetChannel(hadc->Instance, LL_ADC_OFFSET_1)) == __LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel)) {
				LL_ADC_SetOffsetState(hadc->Instance, LL_ADC_OFFSET_1,
						LL_ADC_OFFSET_DISABLE);
			}
			if (__LL_ADC_CHANNEL_TO_DECIMAL_NB(
					LL_ADC_GetOffsetChannel(hadc->Instance, LL_ADC_OFFSET_2)) == __LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel)) {
				LL_ADC_SetOffsetState(hadc->Instance, LL_ADC_OFFSET_2,
						LL_ADC_OFFSET_DISABLE);
			}
			if (__LL_ADC_CHANNEL_TO_DECIMAL_NB(
					LL_ADC_GetOffsetChannel(hadc->Instance, LL_ADC_OFFSET_3)) == __LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel)) {
				LL_ADC_SetOffsetState(hadc->Instance, LL_ADC_OFFSET_3,
						LL_ADC_OFFSET_DISABLE);
			}
			if (__LL_ADC_CHANNEL_TO_DECIMAL_NB(
					LL_ADC_GetOffsetChannel(hadc->Instance, LL_ADC_OFFSET_4)) == __LL_ADC_CHANNEL_TO_DECIMAL_NB(pConfigInjected->InjectedChannel)) {
		...
		if (pConfigInjected->InjectedSingleDiff == ADC_DIFFERENTIAL_ENDED) {
			...
			LL_ADC_SetChannelSamplingTime(hadc->Instance,
					(uint32_t) (__LL_ADC_DECIMAL_NB_TO_CHANNEL(
							(__LL_ADC_CHANNEL_TO_DECIMAL_NB( (uint32_t)pConfigInjected->InjectedChannel) + 1UL) & 0x1FUL)),
					pConfigInjected->InjectedSamplingTime);
		}
	...
	if ((LL_ADC_REG_IsConversionOngoing(hadc->Instance) == 0UL)
		...
		if (pMultimode->Mode != ADC_MODE_INDEPENDENT) {
			MODIFY_REG(tmpADC_Common->CCR, ADC_CCR_MDMA | ADC_CCR_DMACFG,
					pMultimode->DMAAccessMode | ADC_CCR_MULTI_DMACONTREQ((uint32_t)hadc->Init.DMAContinuousRequests));
