...
#define LL_PWR_ReadReg(__REG__) READ_REG(PWR->__REG__)
	...
	__STATIC_INLINE uint32_t LL_PWR_IsEnabledSRAM2Retention(void) {
		return ((READ_BIT(PWR->CR3, PWR_CR3_RRS)
				== (LL_PWR_FULL_SRAM2_RETENTION)) ? 1UL : 0UL);
	}
	...
	__STATIC_INLINE uint32_t LL_PWR_IsEnabledExtSMPS_0V95(void) {
		return ((READ_BIT(PWR->CR4, PWR_CR4_EXT_SMPS_ON)
				== (PWR_CR4_EXT_SMPS_ON)) ? 1UL : 0UL);
	}
	...
	__STATIC_INLINE uint32_t LL_PWR_IsEnabledGPIOPullUp(uint32_t GPIO,
			uint32_t GPIONumber) {
		return ((READ_BIT(*((__IO uint32_t *)GPIO), GPIONumber)
				== (GPIONumber)) ? 1UL : 0UL);
	}
	...
	__STATIC_INLINE uint32_t LL_PWR_IsActiveFlag_ExtSMPSReady(void) {
		return ((READ_BIT(PWR->SR1, PWR_SR1_EXT_SMPS_RDY)
				== (PWR_SR1_EXT_SMPS_RDY)) ? 1UL : 0UL);
	}
#endif /* PWR_SR1_EXT_SMPS_RDY */
