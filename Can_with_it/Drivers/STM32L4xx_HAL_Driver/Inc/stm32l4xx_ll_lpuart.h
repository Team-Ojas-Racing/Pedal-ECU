...
#define __LL_LPUART_DIV(__PERIPHCLK__, __BAUDRATE__) (uint32_t)\
  (((((uint64_t)(__PERIPHCLK__)*LPUART_LPUARTDIV_FREQ_MUL) + (uint32_t)((__BAUDRATE__)/2U))/(__BAUDRATE__)) \
   & LPUART_BRR_MASK)
	 ...
	 * @retval None
	 */
	__STATIC_INLINE void LL_LPUART_DisableClockInStopMode(
	 ...
	 * @retval State of bit (1 or 0).
	 */
	__STATIC_INLINE uint32_t LL_LPUART_IsClockEnabledInStopMode(
			const USART_TypeDef *LPUARTx) {
	...
	__STATIC_INLINE uint32_t LL_LPUART_GetHWFlowCtrl(
			const USART_TypeDef *LPUARTx) {
		return (uint32_t) (READ_BIT(LPUARTx->CR3,
...
#else
			brrresult = (uint32_t) (((uint64_t) (PeriphClk)
...
#define LL_LPUART_IsEnabledIT_TXE  LL_LPUART_IsEnabledIT_TXE_TXFNF /* Redefinition for legacy purpose */
	 ...
	 * @retval None
	 */
	__STATIC_INLINE void LL_LPUART_DisableDMADeactOnRxErr(
			USART_TypeDef *LPUARTx) {
